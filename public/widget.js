(function () {
  ...
  style.textContent = [
    ...
    '.tab:hover{background:#6d28d9;box-shadow:0 8px 28px rgba(124,58,237,.5)}',
  ...
  function renderPin(point) {
    removeByClass('pin');
    var pin = document.createElement('div');
    pin.className = 'pin';
    pin.style.left = point.x + point.scrollX + 'px';
    pin.style.top = point.y + point.scrollY + 'px';
    pin.innerHTML = '<div class="pin-ring"></div><div class="pin-dot">1</div>';
  ...
  function renderModal() {
    ...
    modal.innerHTML = [
      '<header>',
      '<div class="modal-title"><span class="modal-badge"></span><h2>Leave feedback</h2></div>',
  ...
  function humanizeError(status, body) {
    if (status === 403) return 'This domain is not authorised for project "' + projectId + '". Ask the admin to add it in the project settings.';
    if (status === 404) return 'Project ID "' + projectId + '" was not found. The widget script may be using an outdated project ID.';
    if (status === 422) return 'The feedback form has invalid data. Please fill in the comment field and try again.';
    if (status >= 500) return 'The feedback server encountered an error. Please try again in a moment.';
